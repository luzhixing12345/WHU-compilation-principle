...
/* every recursive call of typing() should pass
   the copy of abs list by  Var_list_ptr
   list_copy (Var_list_pt) 
*/   
...
int unify(Type_ptr t1, Type_ptr t2)
{
	...
	if (t1 == NULL || t2 == NULL) {
        ...
        else return unify(get_left(t1), get_left(t2)) && unify(get_right(t1), get_right(t2));
			}
		}
		return 1;
	}
						...
						if (left != NULL) {
							switch (left->kind) {
								...
								case Typevar:
									...
									if (unify(left->left, right))result = left->right;
									else result = NULL;
									break;
								case Arrow:
									if (unify(left->left, right)) result = left->right;
